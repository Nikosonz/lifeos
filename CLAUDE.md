...
The first real UI pass (2026-07-20) built the whole styling/data foundation from a bare `apps/web` (no CSS framework, no components — see git history before this date) and shipped it as one full vertical slice: Finance (Dashboard, Wallets, Categories, Transactions, Budgets). A second pass (same day) added Tasks (task list with status filter + cursor pagination, create/edit dialog, subtasks, Projects, Labels), and a third pass added Calendar (an Agenda view merging events/task deadlines/holidays, plus an event create/edit dialog with recurrence controls) — both following the exact same conventions below. Notifications UI doesn't exist yet — the nav shows it as a disabled "Soon" row rather than a dead link.
...
- **i18n covers Auth + Finance + Tasks + Calendar** (`fa`/`en` routing, RTL, and full message namespaces — `Login`, `Nav`, `FinanceDashboard`, `Wallets`, `Categories`, `Transactions`, `Budgets`, `Tasks`, `Subtasks`, `TaskProjects`, `TaskLabels`, `Calendar`, `Common`); Notifications has no UI or message namespace yet — extend `apps/web/src/messages/{fa,en}.json` when its UI ships, following the existing namespace-per-page pattern.
