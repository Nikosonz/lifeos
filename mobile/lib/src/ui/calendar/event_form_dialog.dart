...
Future<void> showEventFormDialog(
            ...
            children: [
              ...
              DropdownButtonFormField<CalendarRecurrenceFreq?>(
                ...
                items: const [
                  ...
                  DropdownMenuItem(
                    value: CalendarRecurrenceFreq.DAILY,
                    child: Text('روزانه'),
                  ),
                  DropdownMenuItem(
                    value: CalendarRecurrenceFreq.WEEKLY,
                    child: Text('هفتگی'),
                  ),
                  DropdownMenuItem(
                    value: CalendarRecurrenceFreq.MONTHLY,
                    child: Text('ماهانه'),
                  ),
                  DropdownMenuItem(
                    value: CalendarRecurrenceFreq.YEARLY,
                    child: Text('سالانه'),
                  ),
                ],
                    ...
                    for (var i = 0; i < 7; i++)
                      FilterChip(
                        ...
                        onSelected: (v) => setState(
                          () => v ? byWeekday.add(i) : byWeekday.remove(i),
                        ),
                      ),
                  ],
                ),
  ...
  if (eventId == null) {
    await repo.createEvent(
      CalendarEventCreateInput(
        ...
        recurrenceByWeekday: recurrenceFreq == CalendarRecurrenceFreq.WEEKLY
            ? byWeekday.toList()
            : null,
      ),
    );
    unawaited(
      ref
          ...
          .track(TelemetryEventName.CALENDAR_EVENT_CREATED),
    );
  } else {
