...
## Container sharing policy (general rule)

This ADR's Finance/Notifications choice (each module wires its own repository/service instance independently, rather than importing a sibling module's exported singleton) generalizes to a rule every `container.ts` should follow, stated once here instead of restated with slightly different wording in each module's own composition root:

- **A cheap, stateless repository or service** (a thin wrapper over one Prisma model, no cross-repository composition of its own) — each consuming module wires its **own instance** independently, even when a sibling module already exports one over the same table. This is Calendar's `TaskRepository` precedent, and the same reasoning this ADR already applied to Finance's `NotificationRepository`/`NotificationService`. Two instances over the same stateless table are behaviorally identical to one, and independent wiring means no container ever has to import another module's `container.ts` (no inter-container import ordering or circularity to reason about).
- **A whole composed service with its own multi-repository dependency graph and no per-call state** (e.g. `DashboardService`) — reuse the sibling module's already-built singleton directly instead of re-wiring the same graph a second time. This is the Reports/`DashboardService` precedent: duplicating a multi-repository composition here would be pure duplication with real drift risk if Finance's own wiring ever changes, for zero isolation benefit, since the service holds no per-call state to begin with.

Every `container.ts` should state which of these two categories its own wiring choices fall into by referencing this section, not restate the reasoning inline.
